...
const lowPriceProducts: LowPriceProduct[] = [
  {
...
const lowPriceCategoryMap: Record<string, string[]> = {
...
const ensureCategoryAndSubcategoryIds = async () => {
  const subcategoryIdByPath = new Map<string, number>();
...
export const ensureLowPriceProducts = async () => {
  ...
  for (const product of lowPriceProducts) {
    ...
    if (!categoryId) {
      logger.warn({ product: product.name, category: product.category }, "Skipping product with unknown category");
      continue;
    }
  ...
  logger.info({ created, total: lowPriceProducts.length }, "Ensured low-price catalog items");
};
