...
const receiptIdSchema = zod_1.z.object({
    body: zod_1.z.object({}),
    query: zod_1.z.object({}),
    params: zod_1.z.object({ receiptId: zod_1.z.string().min(1) }),
});
...
const renderReceiptPdf = (doc, receipt, items) => {
    ...
    y += 14;
    doc.text(`Address: ${receipt.order.shippingStreet}, ${receipt.order.shippingCity}, ${receipt.order.shippingCountry}`, 48, y);
    y += 24;
...
router.get("/order/:orderId", (0, validate_1.validate)(orderIdSchema), async (req, res, next) => {
    try {
        const receipt = await (0, receipts_service_1.getReceiptByOrderForUser)(req.params.orderId, req.auth.userId);
        res.json(receipt);
    }
    catch (error) {
        next(error);
...
router.get("/:receiptId", (0, validate_1.validate)(receiptIdSchema), async (req, res, next) => {
    try {
...
router.get("/:receiptId/download", (0, validate_1.validate)(receiptIdSchema), async (req, res, next) => {
    try {
        const receipt = await (0, receipts_service_1.getReceiptByIdForUser)(req.params.receiptId, req.auth.userId);
